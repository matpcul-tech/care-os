...
export async function authVault(req: NextRequest): Promise<VaultAuthResult> {
  ...
  const ccRes = await fetch(
    `${SUPABASE_URL}/rest/v1/care_circle?member_user_id=eq.${user.id}&select=patient_id&limit=1`,
    {
  ...
  const rows = (await ccRes.json()) as Array<{ patient_id: string }>;
  if (rows.length === 0) {
    return { ok: false, status: 403, message: "no care circle membership" };
  }

  return { ok: true, userId: user.id, patientId: rows[0].patient_id, accessToken: token };
}
